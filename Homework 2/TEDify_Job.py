...
#RIPULISCO ULTERIORMENTE WATCH_NEXT
valid_ids_list = [row._id for row in filtered_dataset.select("_id").collect()]
valid_ids_col = array([lit(id_val) for id_val in valid_ids_list])

filtered_dataset = filtered_dataset.withColumn(
    "Watch_next_id",
    array_intersect(col("Watch_next_id"), valid_ids_col)
)

print("Schema dopo la pulizia di Watch_next_id:")
